...
{
	//printf("%s():\r\n", __FUNCTION__);
	if (GPIO_ReadInputDataBit( GPIO_ETHER_INT1, BIT_INT_ENC1 ))
		return 0;
	else
		return 1;
}
...
int get_len_paket(int len)
{
    ...
    //printf("%s(): %d :", __FUNCTION__, statusVector.bits.ByteCount);
...
int ambil_paket(int len, char *packet)
{
	u16 newRXTail;
	...
	#if 1
    int i;
    /* cek dari laptopku, memudahkan debug */
    if (packet[7] == 0x1D)
    {
...
static u16 enc424j600ExecuteOp16(u08 op, u16 data) {
	...
	#endif
	spiPut( op );
	((u08 *) &returnValue)[ 0 ] = spiPut( ((u08*) & data) [0] );
	((u08 *) &returnValue)[ 1 ] = spiPut( ((u08*) & data) [1] );    
...
static u32 enc424j600ExecuteOp32(u08 op, u32 data) 
{
	...
	((u08 *) &returnValue)[ 0 ] = spiPut( ((u08*) &data) [ 0 ] );
	((u08 *) &returnValue)[ 1 ] = spiPut( ((u08*) &data) [ 1 ] );  
	((u08 *) &returnValue)[ 2 ] = spiPut( ((u08*) &data) [ 2 ] );  
